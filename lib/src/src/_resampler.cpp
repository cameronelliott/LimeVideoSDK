...
// On Aug 15, 2016, Rich Geldreich gave Cameron Elliott permissions to release this file under a BSD license via email.
...
#define LANCZOS1_SUPPORT (1.0f)
static Resample_Real lanczos1_filter(Resample_Real t)
{
	if (t < 0.0f)
		t = -t;

	if (t < 1.0f)
		return clean(sinc(t) * sinc(t / 1.0f));
	else
		return (0.0f);
}

#define LANCZOS2_SUPPORT (2.0f)
static Resample_Real lanczos2_filter(Resample_Real t)
{
	if (t < 0.0f)
		t = -t;
...
} g_filters[] =
{
   ...
   { "lanczos1", lanczos1_filter, LANCZOS1_SUPPORT },
   { "lanczos2", lanczos2_filter, LANCZOS2_SUPPORT },
